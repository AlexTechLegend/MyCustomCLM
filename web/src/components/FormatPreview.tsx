...
export function FolderTree({
  ...
  return (
    <div className="rounded-xl bg-ink-950/92 text-ink-200 overflow-hidden">
      ...
      {files.length === 0 ? (
        <p className="px-3.5 py-4 text-[12px] text-ink-500">Select formats to preview the folder.</p>
      ) : (
        <ul className="py-1.5">
          {files.map((f) => {
            ...
            return (
              <li
                key={f.filename}
                className={`flex items-center gap-2.5 px-3.5 py-1.5 text-[12px] font-mono ${on ? 'bg-brand-600/25 text-white' : 'text-ink-300'}`}
              >
...
export function ContentsPreview({ spec }: { spec: Pick<OutputSpec, 'format' | 'filename' | 'lineEnding' | 'includeRoot' | 'keyEncoding'> }) {
  ...
  return (
    <div className="rounded-xl bg-ink-950/92 overflow-hidden">
...
export function FormatPreviewPane({
  ...
  return (
    <div className="grid grid-cols-1 gap-3">
      <div>
        <div className="text-[11px] font-medium uppercase tracking-[0.12em] text-ink-500 mb-1.5">From a folder</div>
        <FolderTree directory={destinationPath || '{cn_safe}'} files={files} highlight={spec.filename} />
      </div>
      <div>
        <div className="text-[11px] font-medium uppercase tracking-[0.12em] text-ink-500 mb-1.5">From its contents</div>
