...
export function Mark({ size = 32, className, onDark = false }: { size?: number; className?: string; onDark?: boolean }) {
  return (
    <svg viewBox="0 0 48 48" width={size} height={size} className={className} aria-hidden="true">
      <path d="M24 3.5 41 9.8v13.4c0 10.2-7.1 18.2-17 21.3C14.1 41.4 7 33.4 7 23.2V9.8Z" fill={onDark ? '#14A19F' : '#0E7C7B'} />
  ...
  return (
    <div className={clsx('flex items-center gap-3', className)}>
      ...
      <div className="flex items-baseline gap-2 leading-none">
        <span className={clsx('text-[22px] font-semibold tracking-tight', onDark ? 'text-white' : 'text-ink-950')}>Vigil</span>
        <span className={clsx('text-[11px] font-medium tracking-[0.18em]', onDark ? 'text-ink-400' : 'text-ink-500')}>CLM</span>
      </div>
    </div>
  );
}
